...
export const VisitedPlaceSchema = defineSolidModelSchema({
  ...
  fields: {
    country: {
      type: FieldType.Key,
      rdfProperty: "y:WikicatMemberStatesOfTheUnitedNations",
    }, // optional if placeType === Country
...
export class VisitedPlaceFactory {
    ...
    if (!VisitedPlace.instance) {
      try {
        ...
        defaultContainerUrl = `${baseURL}${
          defaultContainerUrl ?? "myvisitedplaces/"
        }`;
  ...
  async update(
    pk: string,
    payload: IVisitedPlace
  ): Promise<VisitedPlace | undefined> {
    try {
      const res = await VisitedPlace.findOrFail(pk);
