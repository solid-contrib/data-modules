...
describe("Bookmark", () => {
    ...
    it("The getIndexUrl method should return index url with registry exists in typeIndex", async () => {
        ...
        const res = await Bookmark.getRegistryUrls(session.fetch, session.info.webId!);
    ...
    it("the getAll method should get all bookmarks", async () => {
        ...
        jest.spyOn(Bookmark, "getRegistryUrls").mockReturnValue(Promise.resolve(defaultBookmarksDocUrl));
        ...
        expect(Bookmark.getRegistryUrls).toHaveBeenCalled();
